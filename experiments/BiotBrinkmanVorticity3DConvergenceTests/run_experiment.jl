...
function run_experiment(params)
  outfile = datadir("BiotBrinkmanVorticity3DConvergenceTests",gitdescribe(),
                    savename("BiotBrinkmanVorticity3DConvergenceTests",params,"bson"))
  if isfile(outfile)
    println("$outfile (done already)")
    return nothing
  end
  println("$outfile (running)")
  @unpack nk,order,μ,λ,ν,κ,α,c_0 = params
  dict = Driver.driver(nk, order, μ, λ, ν, κ, α, c_0)
  merge!(dict,params)
